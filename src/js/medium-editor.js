  ...
  MediumEditor.prototype = {
    ...
    pasteHTML: function (html) {
      var elList, workEl, i, fragmentBody, pasteBlock = document.createDocumentFragment(),
          forceTopLevelParagraphs = this.options.forceTopLevelParagraphs;
      ...
      this.wrapTextNodes(fragmentBody);
      ...
      for (i = 0; i < elList.length; i += 1) {
        ...
        if(workEl.tagName.toLowerCase() === 'div' && forceTopLevelParagraphs){
          var el = document.createElement('p');
          el.innerHTML = workEl.innerHTML;
          workEl.parentNode.replaceChild(el, workEl);
        }
    ...
    wrapNodes: function(nodes, wrapper){
      while(nodes.length){
        wrapper.appendChild(nodes.shift());
      }
    },

    wrapTextNodes: function(containerEl){

      // Wrap textnodes just under the root in p tags
      var wrap = [], node, textNode, p, i;
      for(i = 0; i < containerEl.childNodes.length; i++){
        node = containerEl.childNodes[i];
        // Text node
        if(node.nodeType === 3){
          wrap.push(node);
        }else if(wrap.length > 0){
          p = document.createElement('p');
          this.wrapNodes(wrap, p);
          containerEl.insertBefore(p, node);
        }
      }
      if(wrap.length > 0){
        p = document.createElement('p');
